...
struct S {
    ...
    void push(int x) {
        a.pb(x);
        if (m.empty()) m.pb(x);
        else m.pb(std::min(m.back(), x));
    }
    int min() {
        return m.empty() ? inf : m.back();
    }
    int pop() {
        int x = a.back();
        a.pop_back();
...
//     cout << endl;
    ...
    forn(i, n) forn(j, m) if (h[i][j] >= b[i][j]) {
        res += h[i][j] - b[i][j] + 1;
//         fore(d, b[i][j], h[i][j]) {
//             if (!check(i-d+1, j-d+1, i, j)) {
//                 cout << "FAIL " << i << " " << j << " (" << d << ")" << endl;
//                 return 1;
//             }
//         }
    }
