...
struct hash_t
{
    ...
    hash_t(): h1(0), h2(0) {}
    hash_t(i64 nh1, i64 nh2): h1(nh1 % m1), h2(nh2 % m2)
    {
        if (h1 < 0) h1 += m1;
        if (h2 < 0) h2 += m2;
    }
    ...
    hash_t shift(int x)
    {
        return hash_t(h1 * pow1[x], h2 * pow2[x]);
    }
    
    hash_t operator - (const hash_t &h) const
    {
        return hash_t(h1 - h.h1, h2 - h.h2);
    }
