...
struct node {
    ...
    node(int lq, int rq) : l(lq), r(rq) {
        if (l == r) {
            L = R = NULL;
            vals[l].reserve(lfr[l].size());
            vals[l].resize(lfr[l].size());
            forn(i, vals[l].size()) {
                vals[l][i] = lfr[l][i];
            }
            mn = vals[l].empty() ? inf : rp[vals[l].back()];
        } else {
    ...
    void remove(int lq, int rq, int mex) {
        ...
        if (lq <= l && rq >= r) {
            if (l == r) {
                while (!vals[l].empty() && rp[vals[l].back()] <= rq) {
                    res[vals[l].back()] = mex;
                    vals[l].pop_back();
                }
                mn = vals[l].empty() ? inf : rp[vals[l].back()];
...
//     cerr << "Time build: " << clock() / 1000 << " ms" << endl;
